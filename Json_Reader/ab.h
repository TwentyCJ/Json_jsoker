...
public:
	...
	Jreader *Find_array(wchar_t *key)
	{		
		vector<pair<wchar_t *, Jreader *>>::iterator iter;
		for (iter = Arrays.begin(); iter != Arrays.end(); ++iter)
			if (!wcscmp(iter->first,key))
				return iter->second;
		return nullptr;
	}
	wchar_t *Find_char(wchar_t *key)
	{
		map<wchar_t *, wchar_t *>::iterator oiter;
		for (oiter = Objects.begin(); oiter != Objects.end(); ++oiter)
			if (!wcscmp( oiter->first ,key))
				return oiter->second;
		return nullptr;
	}
	bool Is_jobject() { return false; }
...
public:
	...
	bool Is_jobject() { return true; }
	Jobject *operator[](int num)
	{
		if (num < Arrays.size())
			return reinterpret_cast<Jobject *>(Arrays[num]);
		return nullptr;
	}
private:
